...
for k=1:N
    ...
    if k < N/tf || 4*N/tf <= k && k < 5*N/tf
        accion = 2; % Avanzar
    elseif N/tf <= k && k < 2*N/tf || 3*N/tf <= k && k < 4*N/tf || 6*N/tf <= k && k < 7.5*N/tf || 8*N/tf <= k && k < 10*N/tf
        accion = 1; % Rotar
    elseif 2*N/tf <= k && k < 3*N/tf  
        accion = 3; % Rotar
    elseif 4*N/tf <= k && k < 6*N/tf 
        accion = 4; % Rotar    
    elseif 7.5*N/tf <= k && k < 8*N/tf
        accion = 5; % Rotar
    end
    
    % Seleccionar la velocidad lineal y angular corre disociando xd, lo que uería decir es que si llegamos a romper alguna parte del spondiente según la acción
    if accion == 1 % Avanzar
        v = 1*ones(1,N); % Velocidad lineal de referencia (m/s)
        w = 0*ones(1,N); % Velocidad angular de referencia (rad/s)
    elseif accion == 2 % Rotar
        v = 0*ones(1,N); % Velocidad lineal de referencia (m/s)
        w = pi/5*ones(1,N); % Velocidad angular de referencia (rad/s)
    elseif accion == 3 % Rotar
        v = 0*ones(1,N); % Velocidad lineal de referencia (m/s)
        w = pi/1.5*ones(1,N); % Velocidad angular de referencia (rad/s)
    elseif accion == 4 % Rotar
        v = 0*ones(1,N); % Velocidad lineal de referencia (m/s)
        w = pi*ones(1,N); % Velocidad angular de referencia (rad/s)
    elseif accion == 5 % Rotar
        v = 0*ones(1,N); % Velocidad lineal de referencia (m/s)
        w = (3*pi)*ones(1,N); % Velocidad angular de referencia (rad/s)
    end
